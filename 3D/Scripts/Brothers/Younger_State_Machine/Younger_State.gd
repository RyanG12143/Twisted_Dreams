...
@export var younger_brother: Node3D
...
func Physics_Update(delta: float):
	# Add gravity.
	if !younger_brother.is_on_floor() and enable_gravity:
		younger_brother.velocity.y -= gravity * delta
...
func _process(delta):
	...
	var lerp_position = younger_brother.global_transform.origin + lerp_interval
	...
	if fps > 60:
		...
		mesh.global_transform = younger_brother.global_transform
